import {
  Directive,
  Input
} from '@angular/core';

import {
  Validator,
  NG_VALIDATORS,
  AbstractControl
} from '@angular/forms';

const isbn10Maybe = /^(?:[0-9]{9}X|[0-9]{10})$/;
const isbn13Maybe = /^(?:[0-9]{13})$/;
const factor = [1, 3];

/* validator.js@6.2.0, MIT, start */
export function isISBN(str: any, version: any = '') : boolean {
  if (typeof str !== 'string') {
    return false;
  }
  version = String(version);
  if (!version) {
    return isISBN(str, 10) || isISBN(str, 13);
  }
  const sanitized: any = str.replace(/[\s-]+/g, '');
  let checksum = 0;
  let i: any;
  if (version === '10') {
    if (!isbn10Maybe.test(sanitized)) {
      return false;
    }
    for (i = 0; i < 9; i++) {
      checksum += (i + 1) * sanitized.charAt(i);
    }
    if (sanitized.charAt(9) === 'X') {
      checksum += 10 * 10;
    } else {
      checksum += 10 * sanitized.charAt(9);
    }
    if ((checksum % 11) === 0) {
      return !!sanitized;
    }
  } else if (version === '13') {
    if (!isbn13Maybe.test(sanitized)) {
      return false;
    }
    for (i = 0; i < 12; i++) {
      checksum += factor[i % 2] * sanitized.charAt(i);
    }
    if (sanitized.charAt(12) - ((10 - (checksum % 10)) % 10) === 0) {
      return !!sanitized;
    }
  }
  return false;
}
/* validator.js@6.2.0, MIT, end */

@Directive({
  selector: '[isISBN]',
  providers: [{provide: NG_VALIDATORS, useExisting: IsISBNValidatorDirective, multi: true}]
})

export class IsISBNValidatorDirective implements Validator {
  @Input() isISBN: string;

  validate(control: AbstractControl): { [key: string]: any } {
    return isISBN(control.value, this.isISBN) ? null : { isISBN: false };
  }
}
